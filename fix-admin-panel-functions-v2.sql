-- =====================================================
-- CORRIGIR FUNÇÕES DO PAINEL ADMINISTRATIVO - VERSÃO 2
-- =====================================================
...
DO $$ 
BEGIN
  -- Adicionar is_banned se não existir
  IF NOT EXISTS (
  ...
  -- Adicionar banned_until se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Adicionar is_muted se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Adicionar mute_until se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 7. FUNÇÃO: VERIFICAR E LIMPAR BANS/MUTES EXPIRADOS
