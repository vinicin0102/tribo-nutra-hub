-- =====================================================
-- TABELAS PARA PAGAMENTOS STRIPE (VERSÃO SEGURA)
-- =====================================================
-- Execute este script no SQL Editor do Supabase
-- Este script cria/atualiza as tabelas necessárias para Stripe
-- Versão segura que adiciona colunas se não existirem
...
-- =====================================================
-- 2. CRIAR/ATUALIZAR TABELA DE PAGAMENTOS
-- =====================================================
...
BEGIN
  ...
  -- Adicionar currency se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'payments' 
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.payments 
    ADD COLUMN payment_provider TEXT NOT NULL DEFAULT 'stripe' CHECK (payment_provider IN ('mercadopago', 'doppus', 'stripe', 'manual', 'peper'));
    RAISE NOTICE 'Coluna payment_provider adicionada à tabela payments';
  END IF;

  -- Adicionar payment_provider_payment_id se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'payments' 
    AND column_name = 'payment_provider_payment_id'
  ) THEN
    ALTER TABLE public.payments 
    ADD COLUMN payment_provider_payment_id TEXT;
    RAISE NOTICE 'Coluna payment_provider_payment_id adicionada à tabela payments';
  END IF;

  -- Adicionar payment_provider_customer_id se não existir
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'payments' 
    ...
    RAISE NOTICE 'Coluna payment_provider_customer_id adicionada à tabela payments';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Coluna payment_method adicionada à tabela payments';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Coluna updated_at adicionada à tabela payments';
  END IF;
END $$;
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (
      ...
      AND table_name = 'payments' 
      AND column_name = 'subscription_id'
    ) THEN '✓ Coluna subscription_id em payments'
    ELSE '✗ Coluna subscription_id em payments'
  END as check_subscription_id,
  CASE 
    WHEN EXISTS (
